...
import { Activity, Box, Cloud, Database, Gauge, Globe, Zap } from "lucide-react"
...
const strategies: CompositionStrategy[] = [
  {
    ...
    isolation: 30,
    scalability: 40,
    description: "JavaScript no browser monta a página usando múltiplos scripts. É o SPA clássico onde cada MFE carrega seus assets e se renderiza.",
    bestFor: ["SPAs com muito JavaScript", "Aplicações interativas", "Conteúdo dinâmico"],
    ...
    diagram: {
      nodes: [
        ...
        { id: "shell", label: "Shell.js", icon: Box, x: 50, y: 30, color: "border-primary" },
        { id: "mfe1", label: "MFE A", icon: Box, x: 50, y: 70 },
        { id: "mfe2", label: "MFE B", icon: Box, x: 70, y: 70 },
      ],
      edges: [
        { from: "user", to: "browser", label: "Click" },
        { from: "browser", to: "shell", animated: true, label: "1. Load Shell" },
  ...
  },
  {
    ...
    isolation: 20,
    scalability: 30,
    description: "Servidor monta o HTML combinando múltiplos MFEs. Como um orquestrador que faz SSI (Server-Side Includes) moderno.",
    bestFor: ["SEO importante", "Performance crítica", "Conteúdo semi-estático"],
    avoid: ["Atualizações em tempo real", "Isolamento rigoroso", "Deploy ultra-independente"],
    diagram: {
      nodes: [
        { id: "user", label: "Usuário", icon: Activity, x: 10, y: 50 },
        { id: "browser", label: "Browser", icon: Globe, x: 30, y: 50 },
        { id: "gateway", label: "API Gateway", icon: Cloud, x: 55, y: 50, color: "border-accent" },
  ...
  },
  {
    ...
    isolation: 40,
    scalability: 90,
    description: "CDN/Edge Workers montam a página antes de chegar ao origin. Lambda@Edge, Cloudflare Workers, Vercel Edge Functions.",
    ...
    diagram: {
      nodes: [
        ...
        { id: "edge1", label: "Edge SP", icon: Zap, x: 40, y: 20, color: "border-chart-4" },
        { id: "edge2", label: "Edge NYC", icon: Zap, x: 40, y: 80, color: "border-chart-4" },
  ...
  },
  {
    ...
    isolation: 10,
    scalability: 50,
    description: "Tudo compilado em um único bundle durante o build. Pode ser monorepo ou script que agrega bundles. Como um link-time de C.",
    bestFor: ["Sites estáticos", "Publicações", "Documentação"],
    avoid: ["Deploy independente", "Atualizações frequentes", "Isolamento"],
    diagram: {
      nodes: [
        { id: "build", label: "Build Server", icon: Cloud, x: 20, y: 50, color: "border-chart-2" },
        ...
        { id: "cdn", label: "CDN", icon: Zap, x: 80, y: 50, color: "border-chart-2" },
      ],
      edges: [
    ...
    <section className="mx-auto max-w-5xl px-4 py-16">
      <div className="mb-8">
        ...
        <p className="mt-4 text-lg text-muted-foreground">
          Quando compor MFEs? Explore as quatro estratégias principais baseadas no momento de composição: cliente, servidor, edge ou build.
        </p>
      </div>
        ...
        {strategies.map((strategy) => {
          ...
          return (
            <button
              ...
              title={strategy.name}
            >
              <Icon className={cn("h-6 w-6", selected === strategy.id ? `text-${strategy.color}` : "text-muted-foreground")} />
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-4">
          <MetricCard label="Latência" value={current.latency} inverted color="destructive" />
          <MetricCard label="Complexidade" value={current.complexity} inverted color="chart-4" />
          <MetricCard label="Isolamento" value={current.isolation} color="accent" />
          <MetricCard label="Escalabilidade" value={current.scalability} color="primary" />
        </div>

        {/* Best For / Avoid */}
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <div className="rounded-lg border border-accent/30 bg-accent/5 p-4">
            <h4 className="mb-3 font-mono text-sm font-semibold text-accent">✓ Melhor para</h4>
          ...
          <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4">
            <h4 className="mb-3 font-mono text-sm font-semibold text-destructive">✗ Evitar quando</h4>
            <ul className="space-y-1">
      ...
      {/* Comparison Table */}
      <div className="mt-12">
        <h3 className="mb-4 text-xl font-bold text-foreground">Matriz de Decisão</h3>
        <div className="overflow-x-auto">
          <ComparisonTable
            headers={["Estratégia", "Latência", "Complexidade", "Isolamento", "Escalabilidade", "Best Use"]}
            rows={strategies.map((s) => [
              s.name.split("(")[0].trim(),
              s.latency <= 30 ? "🟢 Baixa" : s.latency <= 60 ? "🟡 Média" : "🔴 Alta",
              s.complexity <= 40 ? "🟢 Baixa" : s.complexity <= 70 ? "🟡 Média" : "🔴 Alta",
              s.isolation >= 80 ? "🟢 Alto" : s.isolation >= 40 ? "🟡 Médio" : "🔴 Baixo",
              s.scalability >= 80 ? "🟢 Excelente" : s.scalability >= 50 ? "🟡 Bom" : "🔴 Limitado",
              s.bestFor[0],
            ])}
          />
        </div>
      ...
      <div className="mt-8 rounded-lg border border-primary/30 bg-primary/10 p-6">
        <h3 className="mb-4 font-mono text-lg font-semibold text-primary">📌 Recomendação</h3>
        <p className="text-sm text-foreground">
          {selected === "server" && "Server-Side Composition prioriza SEO e performance inicial. Perfeito para e-commerce e conteúdo editorial onde o primeiro paint importa mais que a interatividade."}
          {selected === "client" && "Client-Side é simples de começar mas escalabilidade fica complicada. Apenas para SPAs pequenas ou ferramentas internas."}
          {selected === "edge" && "Edge é o futuro mas requer infraestrutura. Use quando latência global é crítica (Netflix, NYTimes)."}
          {selected === "build" && "Build-Time é direto: sem complexidade de runtime. Ótimo para sites estáticos, documentação, blogs."}
        </p>
      </div>
    </section>
  )
...
function MetricCard({ label, value, inverted = false, color }: { label: string; value: number; inverted?: boolean; color: string }) {
