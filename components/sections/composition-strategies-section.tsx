...
const strategies: CompositionStrategy[] = [
  {
    ...
    description: "Os MFEs são unidos durante o build. O resultado é um artefato único. É como compilar um binário em Go/C++ com todas as dependências inclusas.",
    bestFor: ["Sistemas pequenos", "Baixa frequência de atualização"],
    avoid: ["Times independentes", "Ecossistemas com 10+ MFEs"],
    diagram: {
        nodes: [
          { id: "ci", label: "Pipeline CI/CD", icon: Cloud, x: 20, y: 50, color: "border-emerald-500" },
          { id: "mfe1", label: "MFE A", x: 45, y: 30 },
          { id: "mfe2", label: "MFE B", x: 45, y: 70 },
          { id: "bundle", label: "Artifact Único", icon: Zap, x: 80, y: 50, color: "border-emerald-500" },
        ],
        edges: [
  ...
  },
  {
    ...
    description: "O servidor orquestrador monta o HTML buscando fragmentos de outros serviços. O usuário recebe o documento pronto (SSR).",
    bestFor: ["SEO Crítico", "E-commerce", "Performance inicial"],
    avoid: ["Altíssima interatividade no client", "Apps Offline"],
    diagram: {
        nodes: [
            { id: "browser", label: "Browser", icon: Globe, x: 15, y: 50 },
            { id: "server", label: "MFE Orchestrator", icon: Server, x: 50, y: 50, color: "border-blue-500" },
            { id: "mfe1", label: "UI Service A", x: 85, y: 30 },
            { id: "mfe2", label: "UI Service B", x: 85, y: 70 },
        ],
        edges: [
            ...
            { from: "server", to: "browser", label: "Compiled HTML", animated: true },
        ]
    }
  },
  {
    ...
    description: "O browser baixa o Host que então orquestra o carregamento dos remotos via rede. É a base do Module Federation.",
    ...
    diagram: {
        nodes: [
          ...
          { id: "shell", label: "App Shell", icon: Zap, x: 50, y: 50, color: "border-amber-500" },
          { id: "mfe1", label: "Remote MFE A", x: 85, y: 30 },
          { id: "mfe2", label: "Remote MFE B", x: 85, y: 70 },
        ],
        edges: [
  ...
  },
  {
    ...
    description: "A composição acontece fisicamente perto do usuário (CDN). Combina baixa latência com dinamismo.",
    bestFor: ["Escala Global", "Personalização Regional", "Alta Disponibilidade"],
    avoid: ["Lógica complexa de escrita em DB", "Ambientes de debug restritos"],
    diagram: {
        nodes: [
          { id: "user", label: "Usuário", x: 15, y: 50 },
          { id: "edge", label: "Edge Node (SP)", icon: Zap, x: 50, y: 50, color: "border-violet-500" },
  ...
  },
  {
    ...
    description: "O backend envia um schema (JSON) que descreve a posição e o tipo dos componentes. O cliente é apenas um renderizador burro.",
    bestFor: ["Apps Mobile", "Mudanças de layout sem deploy de app", "Super Apps"],
    avoid: ["Documentos de texto longo", "SEO"],
    diagram: {
        nodes: [
          { id: "client", label: "Renderer Engine", icon: Smartphone, x: 20, y: 50 },
          { id: "api", label: "SDUI BFF", icon: Server, x: 55, y: 50, color: "border-rose-500" },
    ...
    <section className="mx-auto max-w-5xl px-4 py-16">
      <div className="mb-12">
        ...
        <p className="mt-4 text-lg text-muted-foreground">
          Cada decisão arquitetural muda o <strong>fluxo de dados</strong> e a <strong>infraestrutura</strong> necessária.
        </p>
      </div>
      ...
      <div className="mb-10 grid grid-cols-2 gap-3 sm:grid-cols-5">
        {strategies.map((s) => (
          ...
          >
            <s.icon className={cn("h-6 w-6", selected === s.id ? `text-${s.color}-500` : "text-muted-foreground")} />
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
        ...
        <div className="p-8 grid lg:grid-cols-2 gap-12 border-b border-border">
          ...
          <div className="space-y-4 bg-secondary/5 p-6 rounded-xl border border-border/50 shadow-inner">
            <h4 className="text-[10px] font-bold uppercase text-muted-foreground mb-4 tracking-widest">Performance & Operação</h4>
            <MetricBar label="Latência de Resposta" value={current.metrics.perf} color="bg-emerald-500" />
            <MetricBar label="Custo de Infra" value={current.metrics.infra} color="bg-blue-500" />
            <MetricBar label="Autonomia dos Times" value={current.metrics.dev} color="bg-amber-500" />
          </div>
        </div>

        {/* DIAGRAMA - OCUPA TODA A LARGURA (FULL WIDTH NO CARD) */}
        <div className="bg-zinc-950/50 relative">
          <div className="absolute top-4 left-0 w-full text-center">
            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Diagrama de Sequência e Orquestração</span>
          </div>
        ...
        <div className="p-8 grid md:grid-cols-2 gap-8 bg-zinc-900/10 border-t border-border">
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                    {current.bestFor.map((t, i) => (
                        <span key={i} className="text-[11px] bg-emerald-500/10 text-emerald-600 px-3 py-1 rounded-full border border-emerald-500/20">
                            {t}
                        </span>
                    ))}
                </div>
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                    {current.avoid.map((t, i) => (
                        <span key={i} className="text-[11px] bg-destructive/5 text-destructive/70 px-3 py-1 rounded-full border border-destructive/10">
                            {t}
                        </span>
                    ))}
                </div>
      ...
      <div className="mt-16 overflow-hidden">
        ...
        <ComparisonTable
          headers={["Estratégia", "Perf.", "Custo Infra", "Deploy", "Melhor Uso"]}
