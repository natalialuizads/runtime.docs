      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h5 className="font-bold flex items-center gap-2 text-sm">
              <Layers className="h-4 w-4" /> Call Stack (Pilha)
            </h5>
          ...
          <div className="h-48 border-2 border-primary/20 bg-background/50 rounded-lg flex flex-col-reverse p-2 gap-2 relative overflow-hidden transition-all">
            {stack.map((item, i) => (
              <div
                key={i}
                className="bg-primary/20 border border-primary text-primary px-3 py-2 rounded text-xs font-mono font-bold animate-in slide-in-from-top-2 duration-500"
              >
                {item}
              </div>
            ))}
          ...
          <div className="text-xs text-muted-foreground">
            <strong className="text-primary">Regra:</strong> JS é Single Thread.
            Se tem algo aqui, nada mais acontece.
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <h5 className="font-bold flex items-center gap-2 text-sm">
                <Layout className="h-4 w-4" /> Web APIs
              </h5>
              <span className="text-[10px] bg-secondary px-2 py-0.5 rounded text-muted-foreground">
                Browser/C++
              </span>
            ...
            <div className="h-16 border-2 border-dashed border-accent/30 bg-accent/5 rounded-lg flex items-center justify-center gap-2 transition-all">
              {webApi.map((item, i) => (
                <div
                  key={i}
                  className="bg-accent text-accent-foreground px-2 py-1 rounded text-xs font-mono animate-pulse"
                >
                  {item}
                </div>
              ))}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <h5 className="font-bold flex items-center gap-2 text-sm">
                <Clock className="h-4 w-4" /> Task Queue (Fila)
              </h5>
              <span className="text-[10px] bg-secondary px-2 py-0.5 rounded text-muted-foreground">
                Espera sua vez
              </span>
            ...
            <div className="h-16 border-2 border-secondary bg-secondary/20 rounded-lg flex items-center px-2 gap-2 overflow-x-auto transition-all">
              {queue.map((item, i) => (
                <div
                  key={i}
                  className="bg-secondary-foreground text-secondary px-2 py-1 rounded text-xs font-mono whitespace-nowrap animate-in zoom-in duration-300"
                >
                  {item}
                </div>
              ))}
              {queue.length === 0 && (
                <span className="text-[10px] text-muted-foreground/40 w-full text-center">
                  Vazia
                </span>
              )}
            </div>
        ...
        <div className="flex flex-col justify-center space-y-4 border-l border-border pl-8">
          ...
          <div className="mt-4 p-3 bg-muted rounded text-xs text-muted-foreground italic">
            "O Event Loop é o porteiro que só deixa alguém da{" "}
            <strong>Fila</strong> entrar na <strong>Pilha</strong> se a Pilha
            estiver vazia."
          </div>
        </div>
      </div>
    ...
    <section className="mx-auto max-w-4xl px-4 py-16">
      <div className="mb-8">
        ...
        <p className="text-lg text-muted-foreground">
          "O JavaScript tem apenas um atendente para trabalhar. Entender como
          ele malabariza tarefas é a chave para apps fluidos."
        </p>
      </div>
        ...
        <div className="bg-muted/30 p-5 rounded-xl border border-border">
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            <li className="flex items-start gap-2">
              ...
              <span>
                Cada request cria uma nova <strong>Thread</strong>.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-orange-500">•</span>
              <span>
                Se uma thread trava (I/O), o sistema troca para outra.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-orange-500">•</span>
              <span>
                Multitarefa <strong>Preemptiva</strong>.
              </span>
            </li>
          </ul>
        </div>
        ...
        <div className="bg-primary/5 p-5 rounded-xl border border-primary/20">
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            <li className="flex items-start gap-2">
              ...
              <span>
                Apenas <strong>UMA Thread</strong> principal.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-primary">•</span>
              <span>
                Se ela trava, <strong>a página inteira congela</strong>.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-primary">•</span>
      ...
      <div className="mt-12">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <div className="flex-1 text-sm text-muted-foreground">
            <p className="mb-4">
              Para uma animação parecer fluida (60 FPS), o browser precisa
              pintar a tela a cada <strong>16.6ms</strong>.
            </p>
            <p>
              Se o seu código JavaScript na <strong>Call Stack</strong> demorar
              mais que isso, o browser perde a chance de pintar o frame (Frame
              Drop). O usuário sente o "lag".
            </p>
          </div>
          ...
          <div className="flex-1 w-full bg-card border border-border rounded-lg p-4">
            ...
            <div className="flex h-8 w-full rounded overflow-hidden">
              <div
                className="w-[40%] bg-blue-500 flex items-center justify-center text-[10px] text-white font-bold"
                title="JS Execution"
              >
                JS
              </div>
              <div
                className="w-[20%] bg-purple-500 flex items-center justify-center text-[10px] text-white font-bold"
                title="Style Calc"
              >
                Style
              </div>
              <div
                className="w-[20%] bg-green-500 flex items-center justify-center text-[10px] text-white font-bold"
                title="Layout"
              >
                Layout
              </div>
              <div
                className="w-[10%] bg-yellow-500 flex items-center justify-center text-[10px] text-white font-bold"
                title="Paint"
              >
                Paint
              </div>
            ...
            <p className="text-[10px] text-muted-foreground mt-2 text-center">
              Seu código JS deve rodar idealmente em &lt; 10ms para deixar tempo
              para Style, Layout e Paint.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="my-8 rounded-lg border border-primary/30 bg-primary/10 p-6">
        ...
        <p className="text-sm text-primary/80">
          Sempre que tiver um processamento pesado (loop gigante, criptografia,
          processamento de imagem), tire-o da Main Thread. Use Web Workers ou
          quebre em tarefas menores (como visto no desafio anterior).
        </p>
      </div>
    </section>
  );
