...
export function MFEMemorySimulator() {
  ...
  const calculateMemory = () => {
    if (sharedDeps) {
      // Com Module Federation: cada dependencia carrega apenas 1x (versao mais recente)
      ...
      return mfes.reduce((total, mfe) => 
        total + mfe.dependencies.reduce((sum, dep) => sum + dep.size, 0), 0)
    }
  }
  ...
  const optimalMemory = mfes.length > 0 ? 
  ...
  const toggleDependency = (mfeId: string, depName: string) => {
    setMfes(mfes.map(mfe => {
      if (mfe.id !== mfeId) return mfe
      
      const hasDep = mfe.dependencies.some(d => d.name === depName)
      if (hasDep) {
        return { ...mfe, dependencies: mfe.dependencies.filter(d => d.name !== depName) }
      } else {
        ...
        return {
          ...
          dependencies: [...mfe.dependencies, {
            name: dep.name,
            version: dep.versions[Math.floor(Math.random() * dep.versions.length)],
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="font-mono text-sm font-semibold text-primary">MFE Memory Simulator</h3>
        <div className="flex items-center gap-2">
          <span className="font-mono text-xs text-muted-foreground">Module Federation</span>
      ...
      <div className="mb-6 rounded border border-border bg-background p-4">
        <div className="mb-2 flex items-center justify-between">
          <span className="font-mono text-xs text-muted-foreground">Heap Memory Usage</span>
          <span className={`font-mono text-lg font-bold ${sharedDeps ? "text-accent" : "text-destructive"}`}>
        ...
        <div className="flex justify-between font-mono text-xs text-muted-foreground">
          <span>0 MB</span>
          <span className={wastedMemory > 200 && !sharedDeps ? "text-destructive" : ""}>
            {!sharedDeps && wastedMemory > 0 ? `${wastedMemory}KB desperdicados em duplicatas` : "Otimizado"}
          </span>
          <span>2 MB</span>
        </div>
      </div>
      ...
      <div className="mb-4 grid gap-3 md:grid-cols-2">
        {mfes.map(mfe => (
          <div key={mfe.id} className="rounded border border-border bg-background p-3">
            <div className="mb-2 flex items-center justify-between">
              <div>
                <span className="font-mono text-sm font-semibold text-foreground">{mfe.name}</span>
                <span className="ml-2 font-mono text-xs text-muted-foreground">{mfe.team}</span>
              </div>
              <Button size="sm" variant="ghost" onClick={() => removeMFE(mfe.id)}>
            ...
            <div className="flex flex-wrap gap-1">
              {availableDeps.map(dep => {
                const hasDep = mfe.dependencies.find(d => d.name === dep.name)
                return (
                  <button
                    key={dep.name}
                  ...
                  >
                    {dep.name}
                    {hasDep && <span className="ml-1 text-muted-foreground">@{hasDep.version}</span>}
                  </button>
                )
              })}
            </div>
      ...
      <Button onClick={addMFE} variant="outline" className="mb-4 w-full bg-transparent">
      ...
      {!sharedDeps && duplicates.length > 0 && (
        <div className="rounded border border-destructive/30 bg-destructive/10 p-3">
          <div className="mb-2 font-mono text-xs font-semibold text-destructive">Duplicatas Detectadas</div>
          <div className="flex flex-wrap gap-2">
            {duplicates.map(dup => (
              <span key={dup.name} className="rounded bg-destructive/20 px-2 py-1 font-mono text-xs text-destructive">
      ...
      {sharedDeps && (
        <div className="rounded border border-accent/30 bg-accent/10 p-3">
          <p className="font-mono text-xs text-accent">
            <strong>Module Federation ativo:</strong> Dependencias compartilhadas carregam apenas 1x.
            Economia de {wastedMemory}KB comparado ao modo isolado.
          </p>
        </div>
      )}
    </div>
